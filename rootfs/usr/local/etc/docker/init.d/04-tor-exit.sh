...
[ -z "$TOR_EXIT_ENABLED" ] || if [ "$TOR_EXIT_ENABLED" != "yes" ]; then export SERVICE_DISABLED="$SERVICE_NAME" && __script_exit 0; fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
DATA_DIR="/data/tor/exit"   # set data directory
CONF_DIR="/config/tor/exit" # set config directory
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# set the containers etc directory
ETC_DIR="/etc/tor/exit"
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
# set the var dir
VAR_DIR=""
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
TMP_DIR="/tmp/tor"       # set the temp dir
RUN_DIR="/run/tor/exit"  # set scripts pid dir
...
EXEC_CMD_BIN='tor-exit'                # command to execute
EXEC_CMD_ARGS='-f $CONF_DIR/exit.conf' # command arguments
...
TOR_EXIT_ENABLED="${TOR_EXIT_ENABLED:-yes}"
TOR_EXIT_PORT="${TOR_EXIT_PORT:-57005}"
TOR_EXIT_OR_PORT="${TOR_EXIT_OR_PORT:-57006}"
TOR_EXIT_DIR_PORT="${TOR_EXIT_DIR_PORT:-57007}"
TOR_EXIT_NICK_NAME="${TOR_EXIT_NICK_NAME:-}"
TOR_EXIT_ADMIN="${TOR_EXIT_ADMIN:-}"
TOR_EXIT_TOTAL_BANDWIDTH="${TOR_EXIT_TOTAL_BANDWIDTH:-250 GBytes}"
...
__run_precopy() {
	...
	local server_bin="$(type -P "tor-exit" 2>/dev/null)"
	# Define actions/commands
	[ -d "$DATA_DIR" ] || mkdir -p "$DATA_DIR"
	[ -n "$tor_bin" ] && [ -z "$server_bin" ] && cp -Rf "$tor_bin" "/usr/local/bin/tor-exit"
	chmod +x "/usr/local/bin/tor-exit"
...
WARNING: High legal risk - only run if you understand the implications
EOF
	# allow custom functions
...
__update_conf_files() {
	...
	cat <<EOF >"$CONF_DIR/exit.conf"
	# Tor Exit Node Configuration
# WARNING: High legal risk - only run if you understand the implications

##### Exit Configuration for Tor Exit Node
RunAsDaemon 0
HardwareAccel 1
...
ControlSocket $RUN_DIR/exit.sock
CookieAuthFile $RUN_DIR/exit.authcookie
...
##### logging
LogMessageDomains 1
Log warn file $LOG_DIR/warn.log
Log notice file $LOG_DIR/notice.log
#Log debug file $LOG_DIR/debug.log
		
##### Exit Relay Settings
ServerTransportPlugin obfs4 exec /usr/bin/lyrebird
ServerTransportListenAddr obfs4 0.0.0.0:${TOR_EXIT_PORT:-57000}
ORPort ${TOR_EXIT_OR_PORT:-57001}
DirPort ${TOR_EXIT_DIR_PORT:-57002}

ExitRelay 1
PublishServerDescriptor 1
Nickname ${TOR_EXIT_NICK_NAME:-$RANDOM_NICK}
ContactInfo ${TOR_EXIT_ADMIN:-tor-admin@$HOSTNAME}
...
EOF
	...
	if [ "$TOR_DEBUG" = "yes" ]; then
		sed -i 's|#Log debug|Log debug|g' "$CONF_DIR/exit.conf"
	fi
	if [ -z "$HAS_IPV6" ]; then
		sed -i 's|AddressDisableIPv6 0|AddressDisableIPv6 1|g' "$CONF_DIR/exit.conf"
	fi
	# Configure bandwidth using tor-bandwidth script (runs for all services)
	# This is called from the last Tor service init script
	if [ -x "/usr/local/bin/tor-bandwidth" ]; then
		/usr/local/bin/tor-bandwidth configure 2>/dev/null || true
	fi
...
__post_execute() {
	...
	(
		...
		# commands to execute
		sleep 5
		if [ -f "/data/tor/exit/fingerprint" ]; then
			cat "/data/tor/exit/fingerprint"
		fi
		# show exit message
