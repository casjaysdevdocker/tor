...
if [ -n "$TOR_PRIVOXY_ENABLED" ]; then
  if [ "$TOR_PRIVOXY_ENABLED" != "yes" ]; then
...
if [ -f "/config/env/privoxy.script.sh" ]; then
  . "/config/env/privoxy.script.sh"
fi
# Overwrite the variables
if [ -f "/config/env/privoxy.sh" ]; then
  . "/config/env/privoxy.sh"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
