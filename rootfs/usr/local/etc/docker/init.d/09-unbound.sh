...
if [ -n "$TOR_UNBOUND_ENABLED" ]; then
  if [ "$TOR_UNBOUND_ENABLED" != "yes" ]; then
...
if [ -f "/config/env/unbound.script.sh" ]; then
  . "/config/env/unbound.script.sh"
fi
# Overwrite the variables
if [ -f "/config/env/unbound.sh" ]; then
  . "/config/env/unbound.sh"
fi
# - - - - - - - - - - - - - - - - - - - - - - - - -
