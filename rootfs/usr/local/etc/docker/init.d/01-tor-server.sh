...
if [ -n "$TOR_SERVER_ENABLED" ]; then
  if [ "$TOR_SERVER_ENABLED" != "yes" ]; then
...
# set data directory
DATA_DIR="/data/tor/server"
# set config directory
CONF_DIR="/config/tor/server"
# - - - - - - - - - - - - - - - - - - - - - - - - -
...
# - - - - - - - - - - - - - - - - - - - - - - - - -
# set the temp dir
TMP_DIR="/tmp/tor/server"
# set scripts pid dir
RUN_DIR="/run/tor/server"
# set log directory
...
# command to execute
EXEC_CMD_BIN='tor-server'
# command arguments
EXEC_CMD_ARGS='-f $CONF_DIR/server.conf'
...
TOR_SERVER_ENABLED="${TOR_SERVER_ENABLED:-yes}"
...
TOR_SERVER_NICK_NAME="${TOR_SERVER_NICK_NAME:-}"
TOR_SERVER_ADMIN="${TOR_SERVER_ADMIN:-}"
TOR_SERVER_TOTAL_BANDWIDTH="${TOR_SERVER_TOTAL_BANDWIDTH:-250 GBytes}"
